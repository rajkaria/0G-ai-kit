...
> **Deprecated.** Prefer `signEnvelopeWithSigner` — it accepts the `Signer`
> interface and works with KMS / browser wallets. `signEnvelope` continues to
> work but emits a deprecation warning and will be removed in v1.0.
...
[core](/packages/core) (`digestJson`) · [wallet](/packages/wallet) (`signEnvelopeWithSigner`) ·
[da](/packages/da) (digest anchor) · CLI: [`0g attest`](/cli#0g-attest) ·
MCP: `og_attest_verify` ([MCP guide](/mcp)) · React: [`useAttestation`](/react).
