...
describe("setupSdk", () => {
  ...
  it("starts a NodeSDK for the console exporter kind", async () => {
    ...
    // registered, so this is safe to run repeatedly.
    await expect(
      setupSdk({ exporter: { kind: "console" }, serviceName: "sdk-test-console" })
  ...
  it("starts a NodeSDK for the OTLP exporter kind (offline)", async () => {
    ...
    // it only flushes on shutdown / interval. With NO real spans emitted in
    // this test, the URL is never dialled.
    await expect(
      setupSdk({
        exporter: {
          kind: "otlp",
  ...
  it("OBSERVABILITY_EXPORTER_FAILED is a recognised error code", () => {
    ...
    expect(e.helpUrl).toBe(
      "https://0gkit.dev/errors/OBSERVABILITY_EXPORTER_FAILED"
    );
  });
});
